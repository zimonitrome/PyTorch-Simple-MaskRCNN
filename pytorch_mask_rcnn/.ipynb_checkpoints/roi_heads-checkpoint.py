...
def fastrcnn_loss(class_logit, box_regression, label, regression_target):
    if class_logit.shape[-1] == 1:
        classifier_loss = F.binary_cross_entropy_with_logits(class_logit.squeeze(1).float(), label.float())
    else:
...
def maskrcnn_loss(mask_logit, proposal, matched_idx, label, gt_mask):
    ...
    mask_target = torch.ops.torchvision.roi_align(gt_mask, roi, 1., M, M, -1)[:, 0]
